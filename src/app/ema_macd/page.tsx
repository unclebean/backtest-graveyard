...
const SYMBOL = 'EUR';
const STRATEGY = 'ema_macd';
const DESC = 'Forex EUR/USD';
...
function HomePageInternal({ marketData, positions }: Readonly<HomePageProps>) {
  ...
  const macdIndicatorChartRef = useRef<HTMLDivElement>(null);
  ...
  const { macdChart } = useMACDChart({
    candlesticks: candles,
    indicatorChartRef: macdIndicatorChartRef,
  });

  useChartSync([candlestickChart, macdChart], [mainChartContainerRef, macdIndicatorChartRef]);
